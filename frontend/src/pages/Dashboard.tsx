...
// Keep the mock data as a fallback so your UI doesn't break if the backend is empty
const MOCK_CLAIMS = [
  { id: 'CLM-8A9B2C', member_id: 'EMP001', category: 'CONSULTATION', claimed_amount: 1500, status: 'APPROVED', treatment_date: 'Oct 24, 2024' },
  { id: 'CLM-4C0A68', member_id: 'EMP042', category: 'HOSPITALIZATION', claimed_amount: 45000, status: 'MANUAL_REVIEW', treatment_date: 'Oct 23, 2024' },
  { id: 'CLM-9X2Y1Z', member_id: 'EMP018', category: 'PHARMACY', claimed_amount: 850, status: 'APPROVED', treatment_date: 'Oct 21, 2024' },
  { id: 'CLM-7L5M3N', member_id: 'EMP099', category: 'DENTAL', claimed_amount: 5000, status: 'REJECTED', treatment_date: 'Oct 20, 2024' },
];
...
export default function Dashboard() {
  ...
  // --- API Integration (Uncomment when your backend route is ready!) ---
  /*
  useEffect(() => {
    const loadClaims = async () => {
      setIsLoading(true);
      try {
        const data = await api.getRecentClaims(); // You will need to add this to api.ts
        if (data && data.length > 0) {
          setClaims(data);
        }
      } catch (error) {
        console.error("Failed to fetch real claims, falling back to mock data.", error);
      } finally {
        setIsLoading(false);
      }
    };
  ...
  return (
    <div className="min-h-screen bg-[#F8F4EE] text-[#1F1F1F] font-sans p-6 md:p-10">
      ...
      <div className="max-w-6xl mx-auto flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-4">
        ...
        {/* --- NAVIGATION: This button now routes to your Submission form --- */}
        <button 
          onClick={() => navigate('/submit')}
          className="bg-[#E5484D] hover:bg-[#D43B40] text-white px-6 py-3 rounded-xl font-semibold flex items-center gap-2 shadow-sm transition-all transform hover:-translate-y-0.5"
        >
      ...
      <div className="max-w-6xl mx-auto space-y-8">
        ...
        {/* Top Metric Cards (Left unchanged for visual consistency) */}
            ...
            {isLoading ? (
              ...
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-gray-100">
                  {claims.map((claim) => (
                    // --- NAVIGATION: Clicking a row now routes to that specific claim result ---
                    <tr 
                      key={claim.id} 
                      onClick={() => navigate(`/claim/${claim.id}`)}
                      className="hover:bg-[#F8F4EE]/60 transition-colors group cursor-pointer"
                    >
                      <td className="py-4 px-8">
                        <div className="font-mono text-sm text-[#1F1F1F] font-medium">{claim.id}</div>
                        <div className="text-xs text-[#555555] mt-0.5">{claim.member_id || claim.member}</div>
                      </td>
                      <td className="py-4 px-8">
                        <span className="text-sm font-medium text-[#1F1F1F] capitalize">{claim.category?.toLowerCase()}</span>
                      </td>
                      <td className="py-4 px-8">
                        <span className="text-sm font-mono text-[#1F1F1F]">₹{(claim.claimed_amount || claim.amount || 0).toLocaleString()}</span>
                      </td>
                      <td className="py-4 px-8 text-sm text-[#555555]">
                        {claim.treatment_date || claim.date}
                      </td>
                      <td className="py-4 px-8">
